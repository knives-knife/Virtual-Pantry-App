...
//        String hhc = account.getHouseholdCode();
//        Item pNextExpItem = account.getPantry().getNextExpItem(); //may be from db handler
//        Item fNextExpItem = account.getFridge().getNextExpItem(); //may be from db handler
        LocalDateTime pExpDate = LocalDateTime.now().plusDays(3);//pNextExpItem.getExpDate();
        LocalDateTime fExpDate = LocalDateTime.now().plusDays(4);//fNextExpItem.getExpDate();
        DateTimeFormatter format1 = DateTimeFormatter.ofPattern("yyyy-MM-dd", Locale.ENGLISH);
        if (pExpDate.isBefore(fExpDate))
        {
            expDateLabel.setText(format1.format((TemporalAccessor) pExpDate));
            expItemLabel.setText("milk");//(pNextExpItem.getName());
        }
        else
        {
            expDateLabel.setText(format1.format((TemporalAccessor) fExpDate));
            expItemLabel.setText("eggs");//(fNextExpItem.getName());
        }
//        recipes = dbh.get3Recipes(account.getPantry(), account.getFridge());
        String[] recipes = {"Shrimp Alfredo", "Tortilla Soup", "Grilled Cheese"};//del
        recipeButton1.setText(recipes[0]);//.name);
        recipeButton2.setText(recipes[1]);//.name);
        recipeButton3.setText(recipes[2]);//.name);
        
        ArrayList<Member> memList = account.getMemList();
        memList.add(new Member("j2"));
        memList.add(new Member("j3"));
        try 
        {
            hhMemButton1.setText(memList.get(0).getDisplayName());
        }
        catch(Exception e)
        {
        ...
        try 
        {
            hhMemButton2.setText(memList.get(1).getDisplayName());
        }
        catch(Exception e)
        {
        ...
        try 
        {
            hhMemButton3.setText(memList.get(2).getDisplayName());
        }
        catch(Exception e)
        {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new DashboardGUI(new Account(), new Member()).setVisible(true);
            }
        });
    }
